...
import type { ClienteStats } from '../../../utils/clientes_utils';

interface StatDef {
  key: keyof ClienteStats;
  label: string;
  icon: JSX.Element;
  color: string;
  bg: string;
...
const STAT_DEFS: StatDef[] = [
  { key: 'total',               label: 'Total de Clientes',      icon: Icons.user,            color: tokens.color.ferrari, bg: tokens.color.ferrariMid },
  { key: 'emAtendimento',       label: 'Em Atendimento',         icon: Icons.wrench,           color: tokens.color.warn,    bg: tokens.color.warnBg },
  { key: 'aguardandoAprovacao', label: 'Aguardando Aprovação',   icon: Icons.clock,            color: '#1565C0',            bg: '#E3F2FD' },
  { key: 'pagamentoPendente',   label: 'Pagamento Pendente',     icon: Icons.money,            color: tokens.color.crit,    bg: tokens.color.critBg },
  { key: 'prontoRetirada',      label: 'Prontos p/ Retirada',    icon: ClienteIcons.packageCheck, color: '#2E7D32',          bg: '#E8F5E9', scale: 1.15 },
];
