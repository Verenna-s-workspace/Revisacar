        ...
        <div style={grid4}>
          <Field label="Nº da OS *">
            <Input
              ...
              onChangeValue={(value) => setOsHeader((p) => ({ ...p, os_num: value }))}
            />
          </Field>
          <Field label="Data de entrada *">
            <Input
              ...
              onChangeValue={(value) => setOsHeader((p) => ({ ...p, os_date: value }))}
            />
          </Field>
          <Field label="Hora de entrada *">
            <Input
              ...
              onChangeValue={(value) => setOsHeader((p) => ({ ...p, os_time: value }))}
            />
          </Field>
          <Field label="Km atual" hint="ex: 87.450">
            <Input
              ...
              onChangeValue={(value) => setOsHeader((p) => ({ ...p, os_km: value }))}
            />
          </Field>
        </div>
      </FormBlock>
        ...
        <div style={grid3}>
          <Field label="Nome completo *" span={2}>
            <Input
              ...
              value={cliente.nome}
              onChangeValue={(value) => setCliente((p) => ({ ...p, nome: value }))}
            />
          </Field>
          ...
          <Field label="Telefone *">
            <Input
              ...
              onChangeValue={(value) => setCliente((p) => ({ ...p, tel: value }))}
            />
          </Field>
          <Field label="E-mail" span={2}>
            <Input
              ...
              value={cliente.email}
              onChangeValue={(value) => setCliente((p) => ({ ...p, email: value }))}
            />
          </Field>
        </div>
      </FormBlock>
        ...
        <div style={grid4}>
          <Field label="Placa *">
            <Input
              ...
              value={veiculo.placa}
              maxLength={8}
              onChangeValue={(value) => setVeiculo((p) => ({ ...p, placa: value }))}
          ...
          <Field label="Modelo *" span={2}>
            <Input
              ...
              onChangeValue={(value) => setVeiculo((p) => ({ ...p, modelo: value }))}
          ...
          <Field label="Ano">
            <Input
              ...
              placeholder="2014"
              value={veiculo.ano}
              onChangeValue={(value) => setVeiculo((p) => ({ ...p, ano: value }))}
              maxLength={4}
            />
          </Field>
          <Field label="Cor">
            <Input
              ...
              placeholder="Prata"
              value={veiculo.cor}
              onChangeValue={(value) => setVeiculo((p) => ({ ...p, cor: value }))}
            />
          </Field>
          ...
          <Field label="Chassi" span={2} hint="17 caracteres">
            <Input
              ...
              onChangeValue={(value) => setVeiculo((p) => ({ ...p, chassi: value }))}
              maxLength={17}
            />
          </Field>
          <Field label="Observações de entrada" span={4}>
            <Textarea
              ...
              onChangeValue={(value) => setVeiculo((p) => ({ ...p, obs_entrada: value }))}
          ...
          <Field label="Relato do cliente" span={4}>
            <Textarea
              ...
              onChangeValue={(value) => setVeiculo((p) => ({ ...p, obs_cliente: value }))}
    ...
    <StepWrapper direction={stepDir}>
      <FormBlock title="Selecionar Sistemas para Inspeção">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <p style={{ fontFamily: tokens.fontMono, fontSize: "0.72rem", color: tokens.color.muted }}>
            {selected.size === 0
              ? "Selecione ao menos um sistema"
              : `${selected.size} de ${SECTIONS.length} sistemas selecionados`}
          </p>
          <button onClick={onToggleAll} style={{ ...btnGhost, fontSize: "0.65rem", padding: "6px 14px" }}>
          ...
          {SECTIONS.map((sec) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ color: active ? tokens.color.ferrari : tokens.color.muted, display: "flex", alignItems: "center", flexShrink: 0, transition: "color 0.15s" }}>
                  <SectionIcon id={sec.id} size={15} />
                </span>
                <div>
                  <div style={{ fontFamily: tokens.fontMono, fontSize: "0.7rem", fontWeight: 500, letterSpacing: "0.05em", textTransform: "uppercase", color: active ? tokens.color.text : tokens.color.textSecond, transition: "color 0.15s" }}>
                    {sec.label}
                  </div>
                  <div style={{ fontFamily: tokens.fontMono, fontSize: "0.58rem", color: tokens.color.subtle, marginTop: 2 }}>
      ...
      <PanelFooter
        left={<span style={pfNote}>{selected.size > 0 ? `${selected.size} sistema${selected.size > 1 ? "s" : ""} selecionado${selected.size > 1 ? "s" : ""}` : "Nenhum sistema selecionado"}</span>}
        right={
          <>
            ...
            <button onClick={onNext} disabled={selected.size === 0} style={selected.size === 0 ? btnDisabled : btnAccent}>
              Ir para checklist
              <svg width={10} height={10} viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round">
                <path d="M2 5h6M5.5 2l3 3-3 3" />
              </svg>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 7 }}>
            <span style={{ fontFamily: tokens.fontMono, fontSize: "0.62rem", letterSpacing: "0.1em", textTransform: "uppercase", color: tokens.color.subtle }}>
...
export function Step5({
  ...
  tecnico, setTecnico, showErrors, errors,
  ...
  return (
    <StepWrapper direction={stepDir}>
      {/* Resumo */}
      <div style={{ background: tokens.color.surface, borderBottom: `1px solid ${tokens.color.border}`, padding: "26px 40px" }}>
        <SectionTitle>Resumo da OS</SectionTitle>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, background: tokens.color.border, border: `1px solid ${tokens.color.border}`, borderRadius: tokens.radius.md, overflow: "hidden" }}>
          {summaryRows.map(([k, v, c]) => (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "11px 16px", background: tokens.color.surface, fontSize: "0.85rem" }}>
              <span style={{ color: tokens.color.muted, fontFamily: tokens.fontMono, fontSize: "0.72rem" }}>{k}</span>
              <span style={{ fontWeight: 600, color: c || tokens.color.text, fontFamily: tokens.fontMono, fontSize: "0.8rem" }}>{String(v)}</span>
            </div>
          ))}
        </div>
        ...
        {critItems.length > 0 && (
          <div style={{ marginTop: 22 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {critItems.map((ci, i) => (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "11px 16px", background: tokens.color.critBg, border: `1px solid ${tokens.color.critBorder}`, borderRadius: tokens.radius.md, borderLeft: `2.5px solid ${tokens.color.crit}` }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: tokens.color.crit, marginTop: 6, flexShrink: 0, display: "block" }} />
                  <div style={{ fontSize: "0.85rem" }}>
                    <span style={{ fontFamily: tokens.fontMono, fontSize: "0.58rem", color: tokens.color.crit, textTransform: "uppercase", letterSpacing: "0.07em", display: "block", marginBottom: 2 }}>{ci.sec}</span>
                    <span style={{ color: tokens.color.text }}>{ci.name}</span>
                    {ci.obs && <div style={{ fontSize: "0.78rem", color: tokens.color.muted, marginTop: 3 }}>{ci.obs}</div>}
                  </div>
                </div>
              ))}
            </div>
      ...
      <FormBlock title="Dados do Técnico">
        {showErrors && errors.tec_nome && (
          <div style={{ background: tokens.color.critBg, border: `1px solid ${tokens.color.critBorder}`, borderLeft: `2.5px solid ${tokens.color.crit}`, padding: "10px 14px", marginBottom: 18, fontSize: "0.82rem", color: tokens.color.crit, borderRadius: tokens.radius.md, fontFamily: tokens.fontMono }}>
            Preencha o nome do técnico para gerar o PDF.
          </div>
        )}
        <div style={grid4}>
          <Field label="Nome do técnico *" span={2}>
            <input style={inpStyle(hasErr("tec_nome"))} value={tecnico.nome} onChange={(e) => setTecnico((p) => ({ ...p, nome: e.target.value }))} placeholder="Nome completo" />
          </Field>
          <Field label="Registro / CREA">
            <input style={inp} value={tecnico.registro} onChange={(e) => setTecnico((p) => ({ ...p, registro: e.target.value }))} placeholder="000000" />
          </Field>
          <Field label="Km saída">
            <input style={inp} value={tecnico.km_saida} onChange={(e) => setTecnico((p) => ({ ...p, km_saida: e.target.value }))} />
          </Field>
          <Field label="Data de saída">
            <input style={inp} type="date" value={tecnico.data_saida || nowDate()} onChange={(e) => setTecnico((p) => ({ ...p, data_saida: e.target.value }))} />
          </Field>
          <Field label="Hora de saída">
            <input style={inp} type="time" value={tecnico.hora_saida || nowTime()} onChange={(e) => setTecnico((p) => ({ ...p, hora_saida: e.target.value }))} />
          </Field>
          <Field label="Parecer geral" span={4}>
            <textarea style={{ ...inp, minHeight: 88, resize: "vertical" } as CSSProperties} value={tecnico.parecer_geral} onChange={(e) => setTecnico((p) => ({ ...p, parecer_geral: e.target.value }))} placeholder="Descreva o estado geral do veículo e recomendações..." />
          </Field>
        </div>
      </FormBlock>
      ...
      <div style={{ background: tokens.color.surface, borderBottom: `1px solid ${tokens.color.border}`, padding: "26px 40px" }}>
        ...
        <canvas ref={sigRef} className="sig-canvas" style={{ display: "block", width: "100%", height: 140 }} {...sigHandlers} />
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 9 }}>
          <span style={{ fontFamily: tokens.fontMono, fontSize: "0.6rem", color: tokens.color.subtle }}>Assine com o mouse ou o dedo</span>
          <button onClick={onClearSig} style={{ fontFamily: tokens.fontMono, fontSize: "0.6rem", color: tokens.color.subtle, background: "none", border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.07em", textDecoration: "underline", transition: tokens.transition.fast }}>
            Limpar
          </button>
        </div>
      </div>
